...
function Bike() {
  ...
  const [speedkit, setSpeedkit] = useState("5");
  const [rim, setRim] = useState("26");
  const [suspension, setSuspension] = useState(false);
  const [description, setDescription] = useState("asdsadasdasd");
  const [hourlyvalue, setHourlyvalue] = useState("1");
  const [dailyvalue, setDailyvalue] = useState("1");
  const [latitude, setLatitude] = useState("12345");
  const [longitude, setLongitude] = useState("12345");
  ...
  const bubble_opt_color = (v: BikeProps[]) => {
    let n = v.length;
    let swapped;
    do {
      swapped = false;
